...
class MyCaptureMovie:
    ...
    def get_started(self):
        ...
        if not os.path.exists('../00_素材箱/电影网/1_无密钥/index.m3u8'):
            print('true')
    ...
    async def get_movie_url(self):
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(self.second_m3u8_url, ssl=False) as response:
                content = await response.text(encoding='utf-8')
                async with aiofiles.open('../00_素材箱/电影网/1_无密钥/index.m3u8', 'w', 1, 'utf-8') as f:
                    await f.write(content)

    async def download_all_video(self):
        t1 = time.time()
        semaphore = asyncio.Semaphore(50)
        with open('../00_素材箱/电影网/1_无密钥/index.m3u8', 'r', True, 'utf-8') as f:
            count = 0
            tasks = []
            for line in f:
                ...
                async with signal:
                    ...
                    async with aiohttp.ClientSession(headers=headers) as session:
                        async with session.get(ts_url, ssl=False) as response:
                            ...
                            async with aiofiles.open(f'../00_素材箱/电影网/1_无密钥/{count}.ts', 'wb') as f:
                                await f.write(content)
    ...
    def modify_index_file(self):
        index = 1
        with open('../00_素材箱/电影网/1_无密钥/index.m3u8', 'r', True, 'utf-8') as f1:
            with open('../00_素材箱/电影网/1_无密钥/index_modify.m3u8', 'w', True, 'utf-8') as f2:
                for line in f1:
                    if line.startswith('https'):
                        f2.write(f'/Users/hui99563/Documents/01_编程学习/python/09_爬虫开发第5期/00_素材箱/电影网/1_无密钥/{index}.ts\n')
                        index += 1
                    else:
                        f2.write(line)

    def merge(self, filename='movie'):
        os.chdir('../00_素材箱/电影网/1_无密钥')
        cmd = f'ffmpeg -i index_modify.m3u8 -c copy {filename}.mp4'
        os.system(cmd)


if __name__ == '__main__':
    first_m3u8_url = 'https://new.qqaku.com/20211117/iHVkqQMI/index.m3u8'
